-- Criar tipo enum para categorias de produto (se não existir)
DO $$ BEGIN
...
-- Criar tipo enum para tipo de transação (se não existir)
DO $$ BEGIN
...
-- Políticas RLS para profiles (drop se existir antes de recriar)
...
DROP TRIGGER IF EXISTS update_payment_rates_updated_at ON public.payment_rates;
...
-- Inserir taxas de pagamento padrão (se ainda não existirem)
INSERT INTO public.payment_rates (payment_method, rate_percentage) 
VALUES
  ('credito', 3.5),
  ('debito', 2.0),
  ('pix', 0.0),
